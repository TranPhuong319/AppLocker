...
class LockManager: ObservableObject {
    ...
    func load() {
        ...
        do {
            ...
            showAlert(title: "Error".localized, message: "Cannot_load_configuration_file".localized)
            lockedApps = [:]
        }
            ...
            do {
                ...
                if FileManager.default.fileExists(atPath: configFile.path) {
                    ...
                    if sendToHelperBatch(readfile){
                        print("Success_unlocked".localized)
                    } else {
                        print("Unsuccessful_unlocking".localized)
                    }
                }
                ...
                if sendToHelperBatch(savefile){
                    print("✅_Success_of_file_lock".localized)
                } else {
                    print("❌_Lock_the_file_unsuccessful".localized)
                }
            } catch {
                showAlert(title: "Error".localized, message: "Unable_to_save_configuration".localized)
            }
        }
    }
...
//                let markerPathunlock = "\(disguisedAppPath)/Contents/Resources/.locked_\(disguisedAppName).app"
                ...
                guard let infoPlist = try? NSDictionary(contentsOf: appURL.appendingPathComponent("Contents/Info.plist"), error: ()) as? [String: Any],
                    ...
                    print("⚠️_Cannot_read_info.plist_for_%@".localized(with: path))
                    continue
                }
    ...
    func sendToHelperBatch(_ commandList: [[String: Any]]) -> Bool {
            ...
            if let args = cmd["args"] as? [String] {
                for path in args {
                    if path.hasPrefix("/") && !FileManager.default.fileExists(atPath: path) {
                        print("⚠️_File_does_not_exist_%@".localized(with: path))
                    }
                }
            }
        }
        ...
        let proxy = conn.remoteObjectProxyWithErrorHandler { error in
            ...
            DispatchQueue.main.async {
                ...
                alert.informativeText = "Error_happened_when_executing_orders_Details_%@".localized(with: error as CVarArg)
        ...
        proxy?.sendBatch(commandList) { success, message in
            print(success ? "✅_Success".localized : "❌_Failure".localized, message)
            result = success
            semaphore.signal()
        }
