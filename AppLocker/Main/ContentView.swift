...
struct ContentView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            HStack {
                Text("The_application_is_locked".localized)
                    .font(.headline)
                Spacer()
                Button {
                ...
                .help("Add_application_to_lock".localized)
                .disabled(isDisabled)
            }
            .padding(.bottom, 4)
            ...
            if lockedAppObjects.isEmpty {
                ...
                HStack {
                    Spacer()
                    Text("No_application_is_locked.".localized)
...
//                                        .foregroundColor(.secondary)
                    ...
                    if !deleteQueue.isEmpty {
                        Button {
                            ...
                            HStack {
                                ...
                                Text("Waiting_for_%d_task_...".localized(with: deleteQueue.count))
                                    .bold()
                            }
                            .padding()
        ...
        .sheet(isPresented: $showingAddApp) {
            NavigationStack {
                    ...
                    .navigationTitle("Select_the_application_to_lock".localized)
                .toolbar {
                    ToolbarItem(placement: .confirmationAction) {
                        Button(action: lockSelected) {
                            if isLocking {
                                ...
                                Text("Lock_(%d)".localized(with: selectedToLock.count))
                            }
                        }
        ...
        .sheet(isPresented: $showingDeleteQueue) {
            VStack(alignment: .leading) {
                Text("Application_is_waiting_to_delete".localized)
                    .font(.headline)
                    .padding()
                ...
                HStack {
                    Spacer()
                    Button("Delete_all_from_the_waiting_line".localized) {
